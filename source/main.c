...
int main(int argc, char *argv[])
{
     ...
     * Bottom stays double-buffered: botbuttons paints both buffers when dirty
     * so seek/toast overlays do not tear against a live single buffer. */
    ...
    while (aptMainLoop()) {
        ...
        if (!botbuttons_confirm_active() && audio_consume_ended_naturally()) {
            const char *next = musiclist_next_file_after(audio_last_path());
            if (next != NULL)
