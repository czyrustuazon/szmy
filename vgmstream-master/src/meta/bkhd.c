...
VGMSTREAM* init_vgmstream_bkhd(STREAMFILE* sf) {
...

    {
        ...
        if (info)
            snprintf(vgmstream->stream_name, STREAM_NAME_SIZE, "%lu/%s", (unsigned long)subfile_id, info);
        else
            snprintf(vgmstream->stream_name, STREAM_NAME_SIZE, "%lu", (unsigned long)subfile_id);
        if (prefetch)
