...
void describe_vgmstream(VGMSTREAM* vgmstream, char* desc, int length) {
    ...
    snprintf(temp,TEMPSIZE, "sample rate: %ld Hz\n", (long)vgmstream->sample_rate);
    concatn(length,desc,temp);
    ...
    if (vgmstream->channel_layout) {
        ...
        snprintf(temp,TEMPSIZE, "channel mask: 0x%lx /", (unsigned long)vgmstream->channel_layout);
        concatn(length,desc,temp);
    ...
    if (vgmstream->loop_start_sample >= 0 && vgmstream->loop_end_sample > vgmstream->loop_start_sample) {
        ...
        snprintf(temp,TEMPSIZE, "loop start: %ld samples (%1.0f:%06.3f seconds)\n", (long)vgmstream->loop_start_sample, time_mm, time_ss);
        concatn(length,desc,temp);

        describe_get_time(vgmstream->loop_end_sample, vgmstream->sample_rate, &time_mm, &time_ss);
        snprintf(temp,TEMPSIZE, "loop end: %ld samples (%1.0f:%06.3f seconds)\n", (long)vgmstream->loop_end_sample, time_mm, time_ss);
        concatn(length,desc,temp);
    }

    describe_get_time(vgmstream->num_samples, vgmstream->sample_rate, &time_mm, &time_ss);
    snprintf(temp,TEMPSIZE, "stream total samples: %ld (%1.0f:%06.3f seconds)\n", (long)vgmstream->num_samples, time_mm, time_ss);
    concatn(length,desc,temp);
    ...
    if (vgmstream->layout_type == layout_interleave && vgmstream->channels > 1) {
        snprintf(temp,TEMPSIZE, "interleave: %#lx bytes\n", (unsigned long)vgmstream->interleave_block_size);
        concatn(length,desc,temp);

        if (vgmstream->interleave_first_block_size && vgmstream->interleave_first_block_size != vgmstream->interleave_block_size) {
            snprintf(temp,TEMPSIZE, "interleave first block: %#lx bytes\n", (unsigned long)vgmstream->interleave_first_block_size);
            concatn(length,desc,temp);
        }

        if (vgmstream->interleave_last_block_size && vgmstream->interleave_last_block_size != vgmstream->interleave_block_size) {
            snprintf(temp,TEMPSIZE, "interleave last block: %#lx bytes\n", (unsigned long)vgmstream->interleave_last_block_size);
            concatn(length,desc,temp);
        }
    }
    ...
    if (vgmstream->frame_size > 0 || vgmstream->interleave_block_size > 0) {
        ...
        switch (vgmstream->coding_type) {
            ...
            case coding_REF_IMA:
            case coding_PSX_cfg:
                snprintf(temp,TEMPSIZE, "frame size: %#lx bytes\n", (unsigned long)frame_size);
                concatn(length,desc,temp);
                break;
            default:
                break;
    ...
    if (vgmstream->config_enabled) {
        ...
        snprintf(temp,TEMPSIZE, "play duration: %ld samples (%1.0f:%06.3f seconds)\n", (long)samples, time_mm, time_ss);
        concatn(length,desc,temp);
    }
